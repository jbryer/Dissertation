...
latexDescriptives <- function(df, cols, caption=NULL, label=NULL, filename=NULL) {
	r = data.frame()
	for(i in cols) {
		t = cast(as.data.frame(table(df[,i], df$charter, useNA='always')), Var1 ~ Var2, value='Freq')[,1:3]
		p = cast(as.data.frame(prop.table(table(df[,i], df$charter, useNA='always'), 2)), Var1 ~ Var2, value='Freq')[,1:3]
	...
	addtorow$command = c(
		"\\thickline & \\multicolumn{2}{c}{Traditional Schools} & \\multicolumn{2}{c}{Charter Schools} \\\\ \\endfirsthead \\multicolumn{5}{l}{{...continued from previous page}}\\\\ \\hline & \\multicolumn{2}{c}{Charter Schools} & \\multicolumn{2}{c}{Public Schools}  \\\\ \\hline \\endhead",
		paste('\\hline & \\multicolumn{4}{c}{', r[!duplicated(r$Variable),'Variable'], '} \\\\ \\cline{2-5} ', sep='') )
	x = xtable(r[2:6], digits=2, caption=caption, label=label, align='llrrrr')
	print(x, add.to.row=addtorow, floating=FALSE, include.rownames=FALSE, include.colnames=FALSE, caption.placement='top', tabular.environment='longtable', file=filename, hline.after=NULL)
}
